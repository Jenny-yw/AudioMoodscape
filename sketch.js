...
function setup(){
  ...
  function gotSpeech(){
    if (speechRec.resultValue){
      txt.html(speechRec.resultString+" ", true);
      speaking();
    }
  }

  function speaking() {
    var textinput = txt.html();
...
function draw(){
	...
	smooth();
		for(var i = 0; i < nums; i++){
		var radius = map(i, 0, nums, 1, micLevel*600);

		particles_a[i].move();
		particles_a[i].display(radius, score);
          
		particles_b[i].move();
		particles_b[i].display(radius, score);
	} 
}
...
function Particle(x, y){
	...
	this.speed = 0.4;
    this.colorP = color(random(255), 0, 0, 10);
    this.colorN = color(0, 0, random(255), 10);
	this.move = function(){
		var angle = map(noise(this.pos.x/noiseScale, this.pos.y/noiseScale)*TWO_PI*noiseScale, 0, 1000, 0, width);
	...
	this.display = function(r, s){
		// if(s >= 1 && s <= 5) {
		// fill(this.colorP);
		// ellipse(this.pos.x, random(this.pos.y, this.pos.y+3), r, r);}
		// else if(s <= -1 && s >= -5 ) {
		// fill(this.colorN);
		// rect(random(this.pos.x, this.pos.x+6), this.pos.y-5, r+5, r);
		// }
      
      if (s == 1){
        fill(255,215,0, 10);
        ellipse(this.pos.x, random(this.pos.y, this.pos.y+8), r, r);
      }
      if (s == 2){
        fill(255,165,0, 10);
        ellipse(this.pos.x, random(this.pos.y, this.pos.y+6), r, r);
      }
      if (s == 3){
        fill(255,69,0, 10);
        ellipse(this.pos.x, random(this.pos.y, this.pos.y+4), r, r);
      }
      if (s == 4){
        fill(255,0,0, 10);
        ellipse(this.pos.x, random(this.pos.y, this.pos.y+2), r, r);
      }
      if (s == 5){
        fill(220,20,60, 10);
      ...
      if (s == -1){
        fill(173,216,230, 10);
        rect(this.pos.x, this.pos.y-5, r+5, r);
      }
      if (s == -2){
        fill(0,191,255, 10);
        rect(random(this.pos.x, this.pos.x+2), this.pos.y-5, r+5, r);
      }
      if (s == -3){
        fill(30,144,255, 10);
        rect(random(this.pos.x, this.pos.x+4), this.pos.y-5, r+5, r);
      }
      if (s == -4){
        fill(0,0,255, 10);
        rect(random(this.pos.x, this.pos.x+6), this.pos.y-5, r+5, r);
      ...
      if (s == -5){
        fill(0,0,205, 10);
        rect(random(this.pos.x, this.pos.x+8), this.pos.y-5, r+5, r);
      }
}
}
